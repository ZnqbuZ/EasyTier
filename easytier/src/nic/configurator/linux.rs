...
impl PlatformConfigurator for Configurator {
    ...
    async fn remove_ipv4_route(
        ...
        let ifidx = Self::get_interface_index(self.name)?;
    ...
    async fn add_ipv4_ip(
        ...
        message.header.index = IfConfiger::get_interface_index(self.name)?;
        message.header.family = AddressFamily::Inet;
    ...
    async fn set_link_status(&self, up: bool) -> Result<(), Error> {
        ...
        Self::set_flags(self.name, flags)?;
        Ok(())
    }

    async fn set_mtu(&self, mtu: u32) -> Result<(), Error> {
        Self::mtu_op(&self.name, SIOCSIFMTU, mtu as libc::c_int)?;
