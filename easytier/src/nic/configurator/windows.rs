...
impl PlatformConfigurator for Configurator {
    async fn add_ipv4_route(
        ...
        let Some(if_index) = Self::get_interface_index(&self.name) else {
            return Err(Error::NotFound);
        };
        let luid = InterfaceLuid::luid_from_index(if_index).map_err(|e| {
            anyhow::anyhow!("Failed to get interface luid: {}", format_win_error(e))
    ...
    async fn add_ipv4_ip(
        ...
        Self::add_ip_address(&self.name, Ipv4Inet::new(address, cidr_prefix).unwrap()).await
    }

    async fn remove_ipv4_ip(&self, ip: Option<Ipv4Inet>) -> Result<(), Error> {
        Self::remove_ip_address(&self.name, ip).await
    }
    ...
    async fn add_ipv6_ip(
        ...
        Self::add_ipv6_address(&self.name, Ipv6Inet::new(address, cidr_prefix).unwrap()).await
    }

    async fn remove_ipv6_ip(&self, ip: Option<Ipv6Inet>) -> Result<(), Error> {
        Self::remove_ipv6_address(&self.name, ip).await
    }

    async fn set_link_status(&self, up: bool) -> Result<(), Error> {
        Self::set_interface_status(&self.name, up).await
    }
    ...
    async fn set_mtu(&self, mtu: u32) -> Result<(), Error> {
        Self::set_interface_mtu(&self.name, mtu).await
    }
}
