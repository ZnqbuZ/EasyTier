...
use crate::tunnel::common::{PacketBatch, PacketConverter, PacketData};
    ...
    fn convert(&self, packet: ZCPacket) -> Result<PacketBatch, TunnelError> {
        ...
        if !self.gso {
            ...
            if ip.len() > self.mtu as usize && Self::ipv4_tcp_packet_info(&ret, hdr_len).is_some() {
                ...
                return Ok(Self::tcp_segment_ipv4(frozen, hdr_len, self.mtu).into_iter().map(PacketData::Continuous).collect::<Vec<_>>().into());
            }
        }
