...
impl TunStream {
    pub fn new(
        ...
        Self {
            l,
            max_packet_len,
            buf: BufPool::new(max_packet_len << 4),
            margins: BufMargins {
                header,
...
impl Stream for TunStream {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<StreamItem>> {
        ...
        let mut writer = this
            .buf
            .writer(*this.max_packet_len + this.margins.size(), *this.margins);
...
impl TunZCPacketToBytes {
    ...
    fn tcp_segment_ipv4(buf: Bytes, hdr_len: usize, mtu: u16) -> Vec<Bytes> {
    ...
    fn zcpacket_into_bytes(
        ...
        if !self.gso {
            ...
            if ip.len() > self.mtu as usize && Self::ipv4_tcp_packet_info(&ret, hdr_len).is_some() {
                ...
                return Ok(Self::tcp_segment_ipv4(frozen, hdr_len, self.mtu).into());
            }
        }

        tracing::debug!(?payload_offset, "convert zc packet to tun packet");
        Ok([ret.freeze()].into())
    }
}
...
impl VirtualNic {
    ...
    pub async fn create_dev_for_mobile(
        ...
        let ft = TunnelWrapper::new(
            ...
            FramedWriter::new_with_converter(
                TunAsyncWrite { l: b },
                TunZCPacketToBytes::new(has_pi, has_vnet_hdr, mtu, false),
            ),
            None,
        );
    ...
    pub async fn create_dev(&mut self) -> Result<Box<dyn Tunnel>, Error> {
        ...
        let ft = TunnelWrapper::new(
            ...
            FramedWriter::new_with_converter(
                TunAsyncWrite { l: b },
                TunZCPacketToBytes::new(has_pi, has_vnet_hdr, mtu as u16, gso),
            ),
            None,
        );
